...
@interface SGProgressLayer : CALayer
...
@implementation SGProgressLayer
...
@interface SGCircularProgress () <CAAnimationDelegate>

@end

@implementation SGCircularProgress

+ (void) initialize
{
    if (self == [SGCircularProgress class]) {
        SGCircularProgress *circularProgressViewAppearance = [SGCircularProgress appearance];
...
+ (Class)layerClass
{
    return [SGProgressLayer class];
}

- (SGProgressLayer *)circularProgressLayer
{
    return (SGProgressLayer *)self.layer;
}
