...
@interface SGDatePickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
{
...
@implementation SGDatePickerView
...
+ (void)showDatePickerWithTitle:(NSString *)title
    ...
    SGDatePickerView *datePickerView = [[SGDatePickerView alloc]initWithTitle:title dateType:dateType defaultSelValue:defaultSelValue minDate:minDate maxDate:maxDate isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (defaultSelValue && defaultSelValue.length > 0) {
            ...
            if (!defaultSelDate) {
                SGErrorLog(@"参数格式错误！参数 defaultSelValue 的正确格式是：%@", self.selectDateFormatter);
...
- (UIDatePicker *)datePicker {
    if (!_datePicker) {
        _datePicker = [[UIDatePicker alloc]initWithFrame:CGRectMake(0, SG_kTopViewHeight + 0.5, self.alertView.frame.size.width, SG_kPickerHeight)];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (!label) {
        ...
        label.font = [UIFont systemFontOfSize:20.0f * SG_kScaleFit];
        // 字体自适应属性
