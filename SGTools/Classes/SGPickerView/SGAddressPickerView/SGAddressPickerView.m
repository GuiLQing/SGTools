...
@interface SGAddressPickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
{
...
// 选中的省
@property(nonatomic, strong) SGProvinceModel *selectProvinceModel;
// 选中的市
@property(nonatomic, strong) SGCityModel *selectCityModel;
// 选中的区
@property(nonatomic, strong) SGAreaModel *selectAreaModel;
...
@implementation SGAddressPickerView
...
+ (void)showAddressPickerWithShowType:(BRAddressPickerMode)showType
                      ...
                      defaultSelected:(NSArray *)defaultSelectedArr
                         isAutoSelect:(BOOL)isAutoSelect
                          ...
                          resultBlock:(BRAddressResultBlock)resultBlock
                          cancelBlock:(BRAddressCancelBlock)cancelBlock {
    SGAddressPickerView *addressPickerView = [[SGAddressPickerView alloc] initWithShowType:showType dataSource:dataSource defaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)loadData {
    ...
    if (!self.dataSource || self.dataSource.count == 0) {
        /*
            ...
            然后再去找下面名为 SGPickerView 的 bundle 对象。
         */
        NSBundle *bundle = [NSBundle bundleForClass:[self class]];
        NSURL *url = [bundle URLForResource:@"SGPickerView" withExtension:@"bundle"];
        NSBundle *plistBundle = [NSBundle bundleWithURL:url];
        
        NSString *filePath = [plistBundle pathForResource:@"SGCity" ofType:@"plist"];
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        SGProvinceModel *proviceModel = [[SGProvinceModel alloc]init];
        ...
        for (NSDictionary *cityDic in citylist) {
            SGCityModel *cityModel = [[SGCityModel alloc]init];
            ...
            for (NSDictionary *areaDic in arealist) {
                SGAreaModel *areaModel = [[SGAreaModel alloc]init];
...
- (NSArray *)getCityModelArray:(NSInteger)provinceIndex {
    SGProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    // 返回城市模型数组
...
- (NSArray *)getAreaModelArray:(NSInteger)provinceIndex cityIndex:(NSInteger)cityIndex {
    SGProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    SGCityModel *cityModel = provinceModel.citylist[cityIndex];
    // 返回地区模型数组
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        _pickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, SG_kTopViewHeight + 0.5, self.alertView.frame.size.width, SG_kPickerHeight)];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, (self.alertView.frame.size.width) / 3, 35 * SG_kScaleFit)];
    bgView.backgroundColor = [UIColor clearColor];
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(5 * SG_kScaleFit, 0, (self.alertView.frame.size.width) / 3 - 10 * SG_kScaleFit, 35 * SG_kScaleFit)];
    [bgView addSubview:label];
    label.backgroundColor = [UIColor clearColor];
    label.textAlignment = NSTextAlignmentCenter;
    //label.textColor = [UIColor redColor];
    label.font = [UIFont systemFontOfSize:18.0f * SG_kScaleFit];
    // 字体自适应属性
    label.adjustsFontSizeToFitWidth = YES;
    // 自适应最小字体缩放比例
    label.minimumScaleFactor = 0.5f;
    if (component == 0) {
        SGProvinceModel *model = self.provinceModelArr[row];
        label.text = model.name;
    }else if (component == 1){
        SGCityModel *model = self.cityModelArr[row];
        label.text = model.name;
    }else if (component == 2){
        SGAreaModel *model = self.areaModelArr[row];
        label.text = model.name;
    }
    return bgView;
}
...
- (void)showWithAnimation:(BOOL)animation {
    ...
    if (animation) {
        ...
        rect.origin.y = SG_SCREEN_HEIGHT;
        self.alertView.frame = rect;
        // 浮现动画
        [UIView animateWithDuration:0.3 animations:^{
            CGRect rect = self.alertView.frame;
            rect.origin.y -= SG_kPickerHeight + SG_kTopViewHeight + SG_BOTTOM_MARGIN;
            self.alertView.frame = rect;
        }];
    }
}
...
- (SGProvinceModel *)selectProvinceModel {
    if (!_selectProvinceModel) {
        _selectProvinceModel = [[SGProvinceModel alloc]init];
    }
    return _selectProvinceModel;
}

- (SGCityModel *)selectCityModel {
    if (!_selectCityModel) {
        _selectCityModel = [[SGCityModel alloc]init];
...
- (SGAreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        _selectAreaModel = [[SGAreaModel alloc]init];
