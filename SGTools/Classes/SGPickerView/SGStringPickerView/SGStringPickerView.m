...
typedef NS_ENUM(NSInteger, SGStringPickerMode) {
    SGStringPickerComponentSingle,  // 单列
    SGStringPickerComponentMore     // 多列
};

@interface SGStringPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@property (nonatomic, assign) SGStringPickerMode type;
...
@property (nonatomic, copy) SGStringResultBlock resultBlock;
@property (nonatomic, copy) SGStringCancelBlock cancelBlock;

@end

@implementation SGStringPickerView

#pragma mark - 1.显示自定义字符串选择器
+ (void)showStringPickerWithTitle:(NSString *)title
                       dataSource:(id)dataSource
                  defaultSelValue:(id)defaultSelValue
                      resultBlock:(SGStringResultBlock)resultBlock {
...
+ (void)showStringPickerWithTitle:(NSString *)title
                  ...
                  defaultSelValue:(id)defaultSelValue
                     isAutoSelect:(BOOL)isAutoSelect
                       themeColor:(UIColor *)themeColor
                      resultBlock:(SGStringResultBlock)resultBlock
                      cancelBlock:(SGStringCancelBlock)cancelBlock {
    SGStringPickerView *strPickerView = [[SGStringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    if ([[self.dataSourceArr firstObject] isKindOfClass:[NSString class]]) {
        self.type = SGStringPickerComponentSingle;
    } else if ([[self.dataSourceArr firstObject] isKindOfClass:[NSArray class]]) {
        self.type = SGStringPickerComponentMore;
    }
    // 4. 给选择器设置默认值
    if (self.type == SGStringPickerComponentSingle) {
    ...
    } else if (self.type == SGStringPickerComponentMore) {
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.type) {
        case SGStringPickerComponentSingle:
            return 1;
            break;
        case SGStringPickerComponentMore:
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (self.type == SGStringPickerComponentSingle) {
        label.frame = CGRectMake(0, 0, self.alertView.frame.size.width, 35.0f * SG_kScaleFit);
        label.text = self.dataSourceArr[row];
    } else if (self.type == SGStringPickerComponentMore) {
        label.frame = CGRectMake(0, 0, self.alertView.frame.size.width / [self.dataSourceArr[component] count], 35.0f * SG_kScaleFit);
